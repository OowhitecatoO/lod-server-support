...
- **The full v0.7.3 feature set, rebuilt for Minecraft 1.21.11** — This release updates the 1.21.11 support line straight from v0.5.0+mc1.21.11 to the current feature set: everything the main line shipped in v0.6.x–v0.7.3, built for MC 1.21.11. Needs Java 21+ and Fabric API 0.141+; the in-game config screen works on this line (Sodium 0.8.12 is available for 1.21.11).
- **Works with older versions — update in any order** — v0.7.3 uses a newer LOD networking protocol, but it stays fully compatible both ways with the previous release on this line: a v0.7.3 server keeps serving v0.5.0+mc1.21.11 clients (the server translates their protocol on the fly, at their old pace), and a v0.7.3 client renders LODs from a v0.5.0+mc1.21.11 server — including driving generation of new terrain on demand. Update the client, the server, or both — in any order.
