...
- **Anti-xray protection now covers LOD data** — On servers running anti-xray (Paper's built-in, or the AntiXray mod on Fabric — a native 1.21.11 AntiXray build exists), distant LOD columns used to reveal real ore locations. LSS now detects the anti-xray engine automatically and masks the same ores in LOD data; masked chunk data is rebuilt from scratch, so it carries no trace of the hidden blocks. Configurable via `xrayObfuscation` (`"auto"`/`"on"`/`"off"`).
