...
| 13 | Release-line identity | `.github/line.env` values (tag suffix, MC tokens, game-versions, loaders, NeoForge name prose, NeoForge ship flag `LINE_SHIP_NEOFORGE` — mirrored by `release_check.py`'s `SHIP_NEOFORGE`, cross-pinned, make_latest, Java) vs gradle.properties vs the resolved artifact | `ReleaseWorkflowContractTest` + `ToolchainContractTest` (the three-link chain) |
