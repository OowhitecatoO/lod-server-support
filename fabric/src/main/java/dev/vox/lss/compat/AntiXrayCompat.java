 ...
 * max-block-height for LOD masking) is unchanged from the 26.2 primary line, incl. the
 * mode-2/3 {@code ReplacementNoise} discrimination and the transient-null re-probe rung:
 * its reflective surface ({@code Util.getBlockController},
 * {@code ChunkPacketBlockControllerAntiXray}'s {@code obfuscateGlobal}/{@code maxBlockHeight},
 * the Hide controller subclass) exists identically in the 1.21.11 jar. Zero compile-time
 * dependency, mirroring {@code VoxyCompat}.
 *
 * <p>Caveat for future changes: AntiXray 1.4.x's one NON-null-safe mixin path is the
 * {@code LevelChunkSection} deserialization-ctor wrap (raw ThreadLocal read, NPE if
 ...
 * sections through vanilla's chunk-load ctor would silently turn this pass-through into
 * a crash path.
 */
