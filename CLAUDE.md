...
> divergence, palette/data/biomes/light stay byte-exact; same divergence exists between a
> generation-served section and its disk reload on Fabric — one benign extra dirty broadcast
> per waterlogged chunk per session, so "identical wire bytes" claims below carry this ONE
> exception). **The AntiXray crash shim is a documented PASS-THROUGH on this line** — the
> mod's 1.21.11 build threads its context through ThreadLocals (unset get() == null, the
> benign skip value) and ScopedValue is a Java-21 preview API, so `callSerializing` calls
> straight through; the ENGINE PROBE (x-ray mask adoption) is unchanged and fully live.
> The corrupt-region label also differs: 1.21.11 vanilla propagates the ZipException, so a
> corrupt chunk resolves ERROR (not-found on 26.x) — never miss-memoized, and it logs the
> unthrottled "Failed to read chunk NBT" stack, which changes the soak A7 triage below. **`folia-supported: true`** (Folia
...
- `paper/build/libs/lod-server-support-paper.jar` — Paper plugin (server only, shadow JAR; serves Paper, Purpur AND Folia on this line — `folia-supported: true`, experimental)
...
Paper uses `PaperPayloadHandler` with raw `FriendlyByteBuf` encoding. Both produce identical wire format (one pinned 1.21.11 exception: the section count-hint short on waterlogged fixtures — see the branch banner).
...
- **Payloads:** Fabric uses `StreamCodec` with lambda encode/decode registered in `LSSNetworking`; Paper uses raw `FriendlyByteBuf` in `PaperPayloadHandler`. Both produce identical wire bytes (one pinned 1.21.11 exception: the section count-hint short on waterlogged content — see the branch banner).
...
> **SUPPORT-LINE OVERRIDE — read before following the mainline steps below.** On this branch:
> tags are `v<x.y.z>+mc1.21.11` (a bare `v<x.y.z>` collides with MAINLINE tags; since the
> trigger scoping it also simply does not fire this branch's release.yml). Commits land on
> `support/mc1.21.11` directly (NOT via PR to main — step 3 below is mainline-only; this branch
> never merges to main). `-Pmod_version` and `release_check.py --version` take the BARE
> `<x.y.z>` (the workflow strips the `+mc` suffix itself). The release notes file is committed
> at `docs/release-notes-v<x.y.z>+mc1.21.11.md`. Every VSS/voxy/84zcagOb pitfall below is
> mainline-only — this line publishes the LSS pair exclusively, with `make_latest: false`.
